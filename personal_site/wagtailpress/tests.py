...
from wagtailpress.models import BlogIndexPage, BlogArticlePage, BlogArticlePageTag
...
class BlogIndexPageTests(TestCase):
    ...
    def test_all_articles(self):
        ...
        self.assertTrue(descendants_articles.filter(id=self.blog_page_1.id).exists())
        self.assertTrue(descendants_articles.filter(id=self.blog_page_2.id).exists())
    ...
    def test_all_articles_order(self):
        ...
        # Test if we get the 2 pages in the descendants_articles
        self.assertTrue((descendants_articles[0].date > descendants_articles[1].date))

    def test_index_of_article(self):
        """Test if we can get the index of an article"""
        self.assertTrue(self.blog_page_1.index().id == self.blog_index.id)
    ...
    def test_all_articles_tagges_as(self):
        ...
        self.assertTrue(
            self.blog_index.all_articles_tagged_as(self.tags[0])[0].id
            == self.blog_page_1.id
        )
